...
var defaultPrismTheme = themes.palenight;
...
var DocsSchema = Joi.object({
    ...
    sidebar: Joi.object({
        ...
        autoCollapseCategories: Joi.bool().default(DEFAULT_DOCS_CONFIG.sidebar.autoCollapseCategories),
    }).default(DEFAULT_DOCS_CONFIG.sidebar),
}).default(DEFAULT_DOCS_CONFIG);
var DEFAULT_COLOR_MODE_CONFIG = {
    defaultMode: 'light',
    disableSwitch: false,
...
var NavbarItemPosition = Joi.string().equal('left', 'right').default('left');
var NavbarItemBaseSchema = Joi.object({
    label: Joi.string(),
    html: Joi.string(),
    ...
    .unknown();
var DefaultNavbarItemSchema = NavbarItemBaseSchema.append({
    to: Joi.string(),
    href: URISchema,
...
var DocsVersionNavbarItemSchema = NavbarItemBaseSchema.append({
    type: Joi.string().equal('docsVersion').required(),
    to: Joi.string(),
    docsPluginId: Joi.string(),
});
var DocItemSchema = NavbarItemBaseSchema.append({
    type: Joi.string().equal('doc').required(),
    docId: Joi.string().required(),
    docsPluginId: Joi.string(),
});
var DocSidebarItemSchema = NavbarItemBaseSchema.append({
    type: Joi.string().equal('docSidebar').required(),
    sidebarId: Joi.string().required(),
    docsPluginId: Joi.string(),
});
var HtmlNavbarItemSchema = Joi.object({
    className: Joi.string(),
...
var CustomNavbarItemRegexp = /custom-.*/;
var CustomNavbarItemSchema = Joi.object({
    type: Joi.string().regex(CustomNavbarItemRegexp).required(),
}).unknown();
var itemWithType = function (type) {
...
var DropdownSubitemSchema = Joi.object({
    ...
    switch: [
        ...
        {
            is: Joi.alternatives().try(itemWithType('dropdown'), itemWithType('docsVersionDropdown'), itemWithType('localeDropdown'), itemWithType('search')),
...
var DropdownNavbarItemSchema = NavbarItemBaseSchema.append({
    items: Joi.array().items(DropdownSubitemSchema).required(),
});
var DocsVersionDropdownNavbarItemSchema = NavbarItemBaseSchema.append({
...
var LocaleDropdownNavbarItemSchema = NavbarItemBaseSchema.append({
    type: Joi.string().equal('localeDropdown').required(),
    dropdownItemsBefore: Joi.array().items(DropdownSubitemSchema).default([]),
    dropdownItemsAfter: Joi.array().items(DropdownSubitemSchema).default([]),
    queryString: Joi.string(),
});
var SearchItemSchema = Joi.object({
    type: Joi.string().equal('search').required(),
    className: Joi.string(),
});
var NavbarItemSchema = Joi.object({
...
var ColorModeSchema = Joi.object({
    ...
    respectPrefersColorScheme: Joi.bool().default(DEFAULT_COLOR_MODE_CONFIG.respectPrefersColorScheme),
...
var HtmlMetadataSchema = Joi.object({
    id: Joi.string(),
    name: Joi.string(),
    property: Joi.string(),
    content: Joi.string(),
...
var FooterLinkItemSchema = Joi.object({
    to: Joi.string(),
    href: URISchema,
    html: Joi.string(),
    label: Joi.string(),
...
var PrismLanguage = Joi.string().custom(function (val) { return val.toLowerCase(); });
export var ThemeConfigSchema = Joi.object({
    ...
    footer: Joi.object({
        ...
        links: Joi.alternatives(Joi.array().items(Joi.object({
...
export function validateThemeConfig(_a) {
    var validate = _a.validate, themeConfig = _a.themeConfig;
    return validate(ThemeConfigSchema, themeConfig);
}
var DEFAULT_OPTIONS = {
    customCss: [],
};
var PluginOptionSchema = Joi.object({
    customCss: Joi.alternatives()
        .try(Joi.array().items(Joi.string().required()), Joi.alternatives().conditional(Joi.string().required(), {
        then: Joi.custom(function (val) { return [val]; }),
        otherwise: Joi.forbidden().messages({
...
export function validateOptions(_a) {
    var validate = _a.validate, options = _a.options;
    var validatedOptions = validate(PluginOptionSchema, options);
